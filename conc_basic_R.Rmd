...
O **R** é utilizável como uma calculadora, com os operadores aritméticos que se conhece. Para multiplicação utilizamos `*`, para divisão `/`, para expoentes `^` e para raiz quadrada usamos `sqrt()` - do inglês *square root*.
...
A função c() é usada para concatenar valores, criando vetores de valores da mesma classe.
