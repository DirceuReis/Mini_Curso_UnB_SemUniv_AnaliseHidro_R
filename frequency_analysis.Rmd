...
O primeiro passo de estudo de análise de frequência consiste em determinar a distribuição empírica de frequência das vazões máximas anuais observadas, que apresenta a frequência relativa acumulada das observações de uma amostra, e pode ser visto como a versão acumulada do histograma. 

A distribuição empírica de frequência relaciona o valor de cada observação com sua probablidade de não ser excedida, ou seja, 

$$\hat{F}_X(x_i) = P(X \le x_i)$$

em que $\hat{F}_X$ é a estimativa da frequência relativa acumulada da variável aleatória $X$ baseada na amostra, enquanto $x_i$ reprenta o valor de uma dada observação $i$. Existem diversas formas de se determinar $\hat{F}_X(x_i)$ com base com base numa amostra. 

Para entender os motivos pelos quais diversas formas são possíveis, cada uma com a sua motivação e justificativa, precisamos entender que a probabilidade de não excedência de uma esatística de ordem qualquer numa amostra é na verdade uma variável aleatória, já que a mesma é função da amostra que se tem em mãos.
