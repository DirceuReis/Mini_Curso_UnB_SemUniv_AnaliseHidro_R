...
Na verdade, utilizaremos o tempo de recorrência, $T_r$, para plotagem, lembrando que o $T_r$ é simplemenste o inveso da probabilidade de excedência, representada pela posição de plotagem de Weibull.

O código para o cálculo do é apresentado no código abaixo, que cria três colunas ao dataframe que contém as séries de máximos anuais.
...
Como já fizemos antes podemos dar uma espiada nesse dataframe utilizando a fnução `head(dataframe_name)`,
...
Podemos agora cirar uma Figura, siilar àquela apresentada para o estudo do Rio Salado, porém, nesta etapa de nossa análise, apenas com os valores amostrais.

```{r, fig.width = 8, fig.height = 6}

plot(Q_max_ord$Tr,Q_max_ord$maxima,type = "p", pch = 19,log = "xy", 
     main = "Curva de frequência de cheias - 60435000",
