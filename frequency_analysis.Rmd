...
$$
Como $Z\sim N(0,1)$, podemos afirmar que 
...
$$
que em bom português diz que a probabilidade do valor populacional de $W_p$ estar entre esses dois limites, $\hat{W}_p \pm z_{1-\alpha/2}\sqrt{V(\hat{W}_p)}$ é igual à $\alpha$. É essa afirmação matemática que nos permite estabelecer o intervalo de confiança de $100(1-\alpha)\%$ para $W_p$, apresentado abaixo,
...
O problema aqui é que ainda precisamos saber quem é $V(\hat{Y}_p)$. Este não é um curso de estatística, mas sim de `R` aplicado à Hidrologia. Por isso, deixarei a explicação de como determinar $V(\hat{Y}_p)$ num local separado,
