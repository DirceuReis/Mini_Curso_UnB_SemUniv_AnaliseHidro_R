...
Esse processo de maximização é, em muitos casos, realizado por meio do uso de baseado em algum algoritmo baseado no gradiente da função (como Newton-Raphson), e uma transformação que facilita a
convergência desses algoritmos é tirar o logaritmo da função -- sendo log uma função monotônica, essa transformação afeta somente o valor do máximo, mas não o valor de $\hat \theta$. Então, para a GEV, podemos definir a log-verossimilhança como:
