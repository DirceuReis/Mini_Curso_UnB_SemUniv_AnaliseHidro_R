...
Para iniciar essas atividades, vamos primeiro carregar os dados que obtivemos em aulas anteriores.

## Leitura dos dados gerados na última aula

Vamos iniciar nossa exploração dos dados lendo os arquivos gerados quando da obtenção dos dados de vazão via webservide da ANA.

Faremos isso com três arquivos, um contendo apenas a série histórica de uma estação, um segundo objeto contendo os dados da série histórica de todas as estações, e por último, a última variável contendo os dados anuais com valores tratados das estações (mínimo, máximo e média anual). 
...
É comum realizar estudos hidrológicos que contemplem, pelo menos num primeiro momento, um número elevado de estações que possam ser potencialmente utilizadas. 

Para termos uma visão de conjunto dos dados que estão disponíveis, é interessante fazer uma plotagem que permite entender o período de dados disponíveis em cada estação, bem como a ocorrência de falhas nas séries. Essa visão mais geral nos jauda a definir quais estações serão selecionadas, e possivelmente qual o período de dadso que deve ser empreagdo no estudo.

Mas mesmo que estejamos trabalhando cm apenas uma estação, é sempre interessante visualizar o período disponível, bem como as falhas contidas no registro.

Nesta etapa, iniciaremos uma análise de apenas uma estação. Na etapa seguinte, expandiremos para um conjunto de estações. 

Faremos um gráfico do tipo **heatmap**, que é uma figura que coloca cores numa Tabela. No nosso primeiro passo, para cada estação, faremos uma Figura com os anos no eixo-y e os meses no eixo-x, de forma que estaremos representadno as vazões na escala mensal. A tonalidade das cores está associada à magnitude da vazão média do mês em questão, e quando ocorre uma falha, a região do mês correspodente fica branca.
...
Para construir essa Figura inicial de paenas uma estação, vamos trabalhar inicialmente com o `dataframe` denominado `dados_60435000`, cujas primeiras observações podem ser vistas abaixo, onde observamos 3 colunas, o código da estação, a data informando o dia, e o valore da vazão correspondente. 

```{r leitura dos dados}
head(dados_60435000)
```

Para facilitar a construção do **heatmap**, que precisa separa as informações de mês e ano, vamos criar um novo dataframe com mais duas colunas, uma com os meses e a outra com os anos, como pode ser visto na sequência,
...
As duas colunas extras permitirão a realização do cálculo das vazões médias mensais para cada ano, sendo utilizadas as funções `group_by()`, `summarize()` e `mean()`. No final, rearrumamos as linhas em ordem decrescente de ano apenas para criarmos um **heatmap** que coloque os anos iniciais na parte de cima da Figura,
...
Com esse novo_dataframe em mãos, podemos construir a Figura desejada. Mas para utilizar a função `heatmap()`, precisamos fazer uma pequena reestruturação, já que esa função precisa receber uma matriz com os valores de vazão média mensal, organizados de forma que as linhas representem os anos, enquanto as colunas representam os meses. Essa reestruturação é feita com o código abaixo,

```{r reestrutura dataframe}
dados_heatmap <- pivot_wider(data = dados_60435000,
                             names_from = Mes,
                             values_from = Vazao,
                             names_sort = TRUE)
head(dados_heatmap)
...
Agora basta utiliza a função `heatmap()` para construir a Figura desejada. Primeiro apagamos as duas colunas dessa matriz, porque a primeira traz o código da estação e a segunda os anos. Para construção do gráfico abaixo, usando a função `heatmap()`, precisamos apenas de uma matriz com os valores a serem utilizados.
...
Logo na primeira linha, que representa o ano de 1978, podemos ver que não há dados nos 5 primeiros meses, já que temos a cor branca representando os valores de vazão. No restate dos anos, pode-se verificar a ocorrência de falhas em mais três oportunidades: dez/2008, jan/2009 e ago/2019.
